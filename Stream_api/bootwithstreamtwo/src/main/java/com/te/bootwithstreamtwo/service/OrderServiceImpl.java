...
import com.te.bootwithstreamtwo.repository.OrderDAO;
...
//		String category = request.getCategory();
		...
		List<Order> collect = orderDAO.findAll()
							.stream()
...
//		LocalDate date = LocalDate.parse("2022-12-12");
		...
		Double sum = orderDAO.findAll().stream()
				.filter(o -> o.getOrderDate().compareTo(LocalDate.of(2021, 2, 1))>=0)
	...
	public Double dayAverage() {
		...
		Double asDouble = orderDAO.findAll().stream()
				.filter(o -> o.getOrderDate().isEqual(LocalDate.of(2021, 3, 15)))
				.peek(o -> System.out.println(o))
				.flatMap(o -> o.getProducts().stream())
				.mapToDouble(p -> p.getPrice()).average().getAsDouble();
	...
	public Map<Customer, List<Order>> mapCustomer() {
		...
		Map<Customer, List<Order>> collect = orderDAO.findAll().stream().collect(Collectors.groupingBy(Order::getCustomer));
	...
	public Map<Order, Double> mapProductSum() {
		...
		Map<Order, Double> collect = orderDAO.findAll().stream()
				.collect(Collectors.toMap(
						Function.identity(), 
